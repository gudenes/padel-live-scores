...
- **Status:** the console renders **real model odds** from `public.match_odds` (written by the padelgod `odds-computer` worker), delivered via **Supabase Realtime** (`_lib/realtime-provider.ts`). The **stub** (`_lib/stub-provider.ts`) remains the dev fallback behind `NEXT_PUBLIC_LIVE_ODDS_SOURCE=stub` (used by the env-free `/live-odds-preview` route — temporary, remove before the integration PR). Typed contract: `_lib/types.ts`.
- **Model:** ranking pre-match prior (a self-contained mirror of `src/lib/predictions/probability.ts`, in `src/lib/odds/` → mirrored to `padelgod/src/lib/odds/`) **anchored** (binary-search per-point `p`) + an **analytic in-play engine** (`pWinGame`/`pWinSet`/`pWinMatch`, deuce/golden-point/tiebreak, serve-neutral v1). The `odds-computer` worker computes live (~15s) + pre-match and writes `match_odds` + append-only `match_odds_snapshots` (chart history + 15m movement). Spec: `docs/superpowers/specs/2026-05-30-live-odds-model-design.md`; plan: `docs/superpowers/plans/2026-05-30-live-odds-model.md`. Open: serve-split per-point probs, snapshot retention, FK-alias confirmation in the realtime select.
