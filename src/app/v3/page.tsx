    ...
    <div>
      <div style={{ padding: '0 16px', display: 'flex', flexDirection: 'column', gap: 12 }}>
        {grouped.map((group) => {
          ...
          const isExpanded = expandedTournaments[tid] ?? false
          const matchCount = group.matches.length
          const visibleMatches = isExpanded ? group.matches : group.matches.slice(0, 2)
          ...
          return (
            <div key={tid} style={{
              ...
              {/* Tournament header — clickable to expand/collapse */}
              {group.tournament && (
                <button
              ...
              {matchCount > 2 && (
                ...
                >
                  {isExpanded ? 'Show less' : `Show all ${matchCount} results`}
                </button>
              )}
            </div>
          )
  ...
  const fetchData = useCallback(async () => {
    try {
      const [liveRes, scheduledRes, tournamentRes, menRes, womenRes, recentRes, highlightsRes, newsRes] = await Promise.all([
        ...
        supabase.from('highlights').select('id, youtube_id, title, channel_name, thumbnail_url, duration, view_count, published_at, category, allowed_countries, blocked_countries').eq('status', 'active').order('published_at', { ascending: false }).limit(10),
