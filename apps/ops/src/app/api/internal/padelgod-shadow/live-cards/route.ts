// apps/ops/src/app/api/internal/padelgod-shadow/live-cards/route.ts
// GET live match cards for shadow-enabled tournaments.
//
// Scopes:
//   scope=live                  -> only status='live' (no auth required)
//   scope=live+next+recent      -> live + next 6 upcoming + last 6 finished (requires auth)
//
// Optional filter:
//   tournament_id=<uuid>        -> restrict to one tournament
//
// Auth: Auth.js session — isOperator required for scope=live+next+recent (rule 1).
...
export async function GET(request: Request) {
  ...
  // Auth gating: anything beyond scope=live requires operator session
  ...
  const tournamentNames = new Map<string, string>(
    (tournaments as Array<{ id: string; name: string }>).map(t => [t.id, t.name])
  )
  ...
  const activeMatchIds = new Set<string>((activeRows ?? []).map((r: { match_id: string }) => r.match_id))

  // 2b. Find matches in scope.
  ...
  // 4. Fetch shadow points for these matches
  ...
  // 4b. Fetch latest OOP snapshot rows per (tournament_id, court) for player name fallback
  const oopByKey = new Map<string, {
  ...
  for (const row of (oopData ?? []) as OopRow[]) {
    ...
    if (!oopByKey.has(key)) oopByKey.set(key, row)
  }
  ...
  // 5. Build cards with OOP player-name fallback for matches with NULL player FKs
