// apps/ops/src/app/api/internal/tournament-dedup/route.ts
//
...
// ── Inlined from src/lib/source-matcher.ts ─────────────────────────
...
async function buildPlan(supabase: ReturnType<typeof serviceClient>): Promise<PlanResult> {
  const { data, error } = await supabase
    .from('tournaments')
    .select(
  ...
  const fkLookup = dupGroupRows.length > 0
    ? await bulkFkRefs(supabase, dupGroupRows.map(r => r.id))
...
export async function GET() {
  ...
  try {
    const { actions, skippedTooGeneric } = await buildPlan(supabase)
...
export async function POST() {
  ...
  try {
    const result = await buildPlan(supabase)
    plan = result.actions
  } catch (e) {
    return NextResponse.json(
      { ok: false, error: e instanceof Error ? e.message : String(e) },
  ...
  return NextResponse.json({ ok: true, merged, skipped, failed, errors })
}
