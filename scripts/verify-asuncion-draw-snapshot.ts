...
if (!process.env.NEXT_PUBLIC_SUPABASE_URL || !process.env.SUPABASE_SERVICE_KEY) {
  console.error('GATE FAIL: missing NEXT_PUBLIC_SUPABASE_URL or SUPABASE_SERVICE_KEY in env')
  process.exit(1)
}
...
async function main() {
  ...
  const { data: latest } = await s
    ...
    .limit(5)
  if (latest && latest.length > 0) console.table(latest)
  else console.log('(no snapshot rows to display)')
