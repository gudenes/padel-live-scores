  ...
  try {
    const meta = await logOpsEvent('cron:highlights', async () => {
      ...
      if (allVideoIds.size === 0) {
        return { new: 0, channels_checked: PRIORITY_CHANNELS.length, found: 0, hidden: 0, errors: [] }
      }
      ...
      return {
        new: upserted,
        channels_checked: PRIORITY_CHANNELS.length,
    ...
    return NextResponse.json({
      message: (meta.errors?.length ?? 0) > 0 ? 'Highlights sync had errors' : 'Highlights sync complete',
      ...meta,
    })
  } catch (error) {
