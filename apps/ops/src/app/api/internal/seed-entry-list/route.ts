// apps/ops/src/app/api/internal/seed-entry-list/route.ts
...
//
// Ported from src/app/api/ops/seed-entry-list/route.ts.
// Auth: next-auth session + isOperator flag (rule 1).
// Supabase: serviceClient() (rule 2).
// toIso2 → normalizeCountry from @/lib/country (rule 4 — no fip-scraper import).
...
export async function POST(request: Request) {
  ...
  for (const player of body.players) {
    try {
      if (player.action === 'link' && player.playerId) {
        ...
        const iso2 = normalizeCountry(player.country)
  ...
  // Write pairs to tournament_draws
  for (let i = 0; i < body.players.length; i += 2) {
    ...
    const { error: upsertError } = await supabase
      ...
      .upsert({
        ...
        player1_country: normalizeCountry(p1.country),
        player1_id: resolvedIds[i],
        player2_name: p2?.name ?? '',
        player2_country: p2 ? normalizeCountry(p2.country) : null,
