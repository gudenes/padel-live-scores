...
export default function UnresolvedPartnerModal({ ctx, onClose, onResolved }: UnresolvedPartnerModalProps) {
  ...
  // Debounced search whenever query or ctx changes
  useEffect(() => {
    if (!ctx || !query.trim()) {
      setHits([])
      return
    ...
    const t = setTimeout(async () => {
      ...
      try {
        const r = await fetch(
          `/api/ops/search-players?q=${encodeURIComponent(query)}&category=${ctx.category}&per_page=25`,
        )
    ...
    >
      ...
      >
        ...
        <div style={{ fontSize: 12, color: '#666', marginBottom: 12 }}>
          ...
          ({ctx.category}
          {ctx.countryHint ? `, ${ctx.countryHint}` : ''})
        </div>
        ...
        {tab === 'link' && (
          <div>
            ...
            >
              {searching && (
                <div style={{ padding: 8, fontSize: 12, color: '#999' }}>Searching…</div>
              )}
