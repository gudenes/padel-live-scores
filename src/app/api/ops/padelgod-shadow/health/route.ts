// src/app/api/ops/padelgod-shadow/health/route.ts
// GET aggregated health metrics for the Padelgod Shadow Mode ops card.
// Auth: ops_token cookie (httpOnly, set by proxy on /ops login).
...
export async function GET() {
  ...
  if (pollErr) {
    ...
    return Response.json({ error: pollErr.message }, { status: 500 })
  }
  ...
  const livePollSuccessPct =
    pollRows.length === 0
      ? null
      : Math.round(
          (100 * pollRows.filter(r => r.status === 'success').length) / pollRows.length
        )
  ...
  if (unresErr) {
    ...
    return Response.json({ error: unresErr.message }, { status: 500 })
  }
  ...
  const finalStateMatchPct =
    finals.length === 0
      ? null
      : Math.round(
          (100 * finals.filter(r => r.winner_match === true && r.score_match === true).length) /
            finals.length
        )
  ...
  if (ppErr) {
    ...
    return Response.json({ error: ppErr.message }, { status: 500 })
  }
  const pps = ppRows ?? []
  const perPointMatchPct =
    pps.length === 0
      ? null
      : Math.round((100 * pps.filter(r => r.point_sequence_match === true).length) / pps.length)
  ...
  if (latErr) {
    ...
    return Response.json({ error: latErr.message }, { status: 500 })
  }
