...
// World padel player rankings — Men & Women tabs
...
function PlayerRow({ player, onClick }: { player: Player; onClick: () => void }) {
  ...
  // delta placeholder — would come from a previous_ranking column in the future
  const delta = 0
    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{
          ...
          {player.country
            ? <><span>{countryFlag(player.country)}</span><span>{player.country}</span></>
...
export default function RankingPage() {
  ...
  const load = useCallback(async (category: 'men' | 'women') => {
    ...
    // Latest updated_at across all fetched players = "ranking last updated"
    ...
    <div style={{ maxWidth: 500, margin: '0 auto', paddingBottom: 20 }}>
      ...
      <div style={{ padding: '20px 16px 12px', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
        ...
        <div style={{
          ...
          <button style={tabStyle(tab === 'men')}   onClick={() => setTab('men')}>Men</button>
          <button style={tabStyle(tab === 'women')} onClick={() => setTab('women')}>Women</button>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div style={{ padding: '60px 20px', textAlign: 'center' }}>
          ...
          <p style={{ color: 'var(--text-muted)', fontWeight: 600, fontSize: 15 }}>No rankings yet</p>
