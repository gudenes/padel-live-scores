 ...
 *   2. Pair-based fallback — match on unordered {pair1, pair2} player UUIDs
 *      within (tournament_id, category, round). Prevents duplicate match rows
 *      when the draw-reconciler created the row before the widget id was known.
 *      Skipped when any of the four player UUIDs is null.
 *   3. Fresh INSERT into `matches` + link via `entity_external_ids` using
