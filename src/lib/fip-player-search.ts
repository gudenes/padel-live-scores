...
export async function searchFipPlayer(
  ...
  const pick = (hits: RawSearchHit[]): FipPlayerSearchResult | null => {
    ...
    // Reject hits whose full name doesn't share enough tokens with the input.
    // Without this guard the surname-only candidate query (e.g. "Camacho"
    // derived from "Nuria Rodriguez Camacho") returns whoever shares that
    // surname (e.g. "Elvira Camacho Marente"), and the country/gender
    // filters happily pass them through.
    pool = pool.filter(h => isPlausibleNameMatch(input.name, `${h.name} ${h.surname}`.trim()))
